                    ...
                    if (filename.endsWith('src/app/page.tsx')) {
                        node.body.forEach((statement) => {
                            if (statement.type === "ExpressionStatement" &&
                                ...
                                context.report({
                                    node: statement,
                                    message: 'Top-level "use client" directive may cause problems to existing stack.You\'re importing a component that needs server-only. That only works in a Server Component which is not supported in the pages/ directory.',
                                });
                            }
                        });
                    }
